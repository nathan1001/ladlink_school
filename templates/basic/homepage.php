<?php
	/*
		Resources Available:
		$hero_images = hero - Media Gallery
		$school_menu_card = school menu card - Matrix
		$head_image = headmistress image - Image Upload
		$headmistress_card = headmistress card - Matrix
		$calendar_event = calendar event - Matrix
		$ladlink_glance_one = ladlink glance one - Matrix
		$ladlink_glance_two = ladlink glance two - Matrix
		$ladlink_glance_three = ladlink glance three - Matrix
	*/
	//print_r($headmistress_card )
?>
	<div class="hero-section uk-position-relative uk-visible-toggle uk-dark" tabindex="-1" uk-slider="center: true; autoplay: true;autoplay-interval:5000">
		...
		<?foreach( $hero_images as $object ){?>
			<li >
				<div class="uk-cover-container">
					<img src="<?= $object['image']?>" alt="" uk-cover>
				</div>
			</li>
			<?}?>
		</ul>
	...
	<section class="uk-section uk-padding-remove school-menu-section" >
		...
		<?foreach( $school_menu_card as $object ){?>
			<div class="school-menu-card">
				<a class="card-link" href="http://ladlinkschool.com/academics.html">
					<div class="uk-padding ">
						<div class="uk-text school-menu-text-title"><?= $object['schooldepartment']?></div>
						<p class="uk-text school-menu-text-body"><?= $object['schooldescription']?></p>
						<div class="school-menu-text-age">
							<div class=" school-age-number splash-burgundy"><?= $object['schoolagerange']?></div> <div class="superscript splash-burgundy">years old</div>
						</div>
	...
	<section class="uk-section uk-padding-remove uk-margin-large-top" >
			...
			<div class="home-head" style="background-image: url('<?=$head_image?>')">
				
			</div>
			<div class="message-box">
				<div class="uk-padding uk-text mobile-padding">
					<div class="underline-header column-title"><?=$headmistress_card[0]['title']?></div>
					<p class="text-body">
					<?=$headmistress_card[0]['messagebody']?>
					</p>
				</div>
			</div>
			...
			<div class=" uk-text-left" >
				<div class="fact-card">
					<div class="fact-header left-fact-padding">
						...
						<div class="fact-title splash-burgundy splash-charter column-header"><?=$ladlink_glance_one[0]['title']?></div>
					</div>
					<div class="fact-body uk-margin-top left-fact-padding" >
						<p class="text-body"><?=$ladlink_glance_one[0]['body']?> </p>
					</div>
				</div>
				...
				<div class="fact-card">
					<div class="fact-header mid-fact-padding " >
						...
						<div class="fact-title splash-burgundy splash-charter column-header"><?=$ladlink_glance_two[0]['title']?></div>
					</div>
					<div class=" uk-margin-top fact-body mid-fact-padding">
						<p class="text-body"><?=$ladlink_glance_two[0]['body']?></p>
					</div>
				</div>
				...
				<div class="fact-card">
					<div class="fact-header right-fact-padding" > 
						...
						<div class="fact-title splash-burgundy splash-charter column-header"><?=$ladlink_glance_three[0]['title']?></div>
					</div>
					<div class="fact-body uk-margin-top right-fact-padding">
						<p class="text-body"><?=$ladlink_glance_three[0]['body']?></p>
					</div>
				</div>					
			</div>
		</div>
